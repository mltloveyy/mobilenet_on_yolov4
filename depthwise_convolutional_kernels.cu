		...
		const int stride, const int pad_rows, const int pad_cols,int num_in_backprop)
{
		...
		const int out_r_start = 0 < (in_r - filter_rows + pad_rows + stride) / stride ? (in_r - filter_rows + pad_rows + stride) / stride : 0;
		const int out_r_end = out_rows - 1 > (in_r + pad_rows) / stride ? (in_r + pad_rows) / stride : out_rows - 1;
		const int out_c_start = 0 < (in_c - filter_cols + pad_cols + stride) / stride ? (in_c - filter_cols + pad_cols + stride) / stride : 0;
		const int out_c_end = out_cols - 1 > (in_c + pad_cols) / stride ? (in_c + pad_cols) / stride : out_cols - 1;
...
void forward_depthwise_convolutional_layer_gpu(depthwise_convolutional_layer l, network_state net)
{
...
void backward_depthwise_convolutional_layer_gpu(depthwise_convolutional_layer l, network_state net)
{
...
void update_depthwise_convolutional_layer_gpu(depthwise_convolutional_layer l, int batch, float learning_rate_init, float momentum, float decay, float loss_scale)
{
