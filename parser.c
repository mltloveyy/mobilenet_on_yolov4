...
#include "depthwise_convolutional_layer.h"  // depthwise convolutional
...
LAYER_TYPE string_to_layer_type(char * type)
{
    ...
    if (strcmp(type, "[dwconv]")==0 || strcmp(type, "[depthwise_convolutional]") == 0) return DEPTHWISE_CONVOLUTIONAL;  // depthwise convolutional
...
network parse_network_cfg_custom(char *filename, int batch, int time_steps)
{
    ...
    while(n){
        ...
        if (lt == CONVOLUTIONAL) {
          ...
          l = parse_depthwise_convolutional(options, params); // depthwise convolutional
        }else if(lt == LOCAL){
...
void load_weights_upto(network *net, char *filename, int cutoff)
{
    ...
    *net->cur_iteration = get_current_batch(*net);
    ...
    for(i = 0; i < net->n && i < cutoff; ++i){
        ...
        if(l.type == DEPTHWISE_CONVOLUTIONAL){ // depthwise convolutional
